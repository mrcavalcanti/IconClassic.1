...
type
  ...
  public
    ...
    procedure SaveToStream2(Stream : TStream; Filename : String);
...
function TAdvancedIcon.LoadFromStream(Stream: TStream): Boolean;
Var
  mStream    : TStream;
  IconCount : Word;
  I         : Integer;
...
begin
  ...
  TMemoryStream(mStream).LoadFromStream(Stream);
  mStream.Seek(0, soFromBeginning);
  with mStream do
  try
    IF NOT LoadHeader(mStream,IconCount) Then exit;
    FOR I:=1 TO IconCount DO begin
      Image:=TFileIconImage.Create(mStream);
      IF Assigned(Image) Then Images.add(Image);
    end;
    Result:=True;
  finally
...
procedure TAdvancedIcon.SaveToStream2(Stream : TStream; Filename : String);
Var
  ms        : TMemoryStream;
begin
  ms := TMemoryStream.Create;
  ms.LoadFromStream(Stream);
  if not (ms.Size > 0) then
    exit;
  SaveToStream(ms);
